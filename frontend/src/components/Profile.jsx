...
function ProfileStory({ image, title, id, rating, author, wordCount, description }) {
    return (
        <div className="profile-story">

            <div className="profile-story-image-column">
                <img
                    src={image}
                    alt="story-cover1"
                    className="profile-story-cover"
                />
            </div>

            <div className="profile-story-info">
                <h2 className="profile-story-title">
                    <Link to={"/discover/story/" + id}>{title}</Link>
                </h2>

                <div className="profile-story-rating">
                    <StarRating rating={rating} size="20px" />
                </div>

                <p className="profile-story-author">
            ...
            try {
                ...
                if (data.username === params.username) {
                    ...
                    const res2 = await fetch(
                        "http://localhost:5000/api/users/byUsername/" + params.username,
                        {
                            credentials: 'include'
                        }
                    );
    ...
    useEffect(() => {
        ...
        async function getWorks() {
            ...
            try {
                ...
                const res = await fetch(
                    "http://localhost:5000/api/stories/author/" + profile._id,
                    {
                        credentials: 'include'
                    }
                );
    ...
    useEffect(() => {
        ...
        async function getReviews() {
            ...
            try {
                ...
                const res = await fetch(
                    "http://localhost:5000/api/users/" + profile._id + "/reviews",
                    {
                        credentials: 'include'
                    }
                );
                ...
                {activeTab === "stories" && (
                    ...
                    <div className="profile-stories">
                        ...
                        {works.length === 0 ? (
                            ...
                            >
                                There are no works to display from this author.
                            </h3>
